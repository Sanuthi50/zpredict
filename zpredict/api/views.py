...
def admin_download_file(request, file_id):
    """Download uploaded PDF file by ID"""
    if not is_admin_user(request.user):
        return Response({"error": "Admin access required"}, status=403)
    ...
    try:
        ...
        if not upload.pdf_file or not default_storage.exists(upload.pdf_file.name):
        ...
        try:
            file_path = upload.pdf_file.path
            ...
            filename = upload.original_filename or f"document_{file_id}.pdf"
            ...
            logger.error(f"Error reading file {upload.pdf_file.name}: {e}")
...
# =============================
# Admin Retry Upload View
# =============================
@api_view(['POST'])
@permission_classes([IsAuthenticated])
def admin_retry_upload(request, file_id):
    """Retry processing of a failed upload"""
    if not is_admin_user(request.user):
        return Response({"error": "Admin access required"}, status=403)
    ...
    try:
        ...
        logger.error(f"Retry error for file {file_id}: {e}")
        return Response({"error": "Retry failed"}, status=500)


# =============================
# Admin Delete Upload View
# =============================
@api_view(['DELETE'])
@permission_classes([IsAuthenticated])
def admin_delete_upload(request, file_id):
    ...
    try:
        ...
        try:
            if upload.pdf_file and default_storage.exists(upload.pdf_file.name):
                default_storage.delete(upload.pdf_file.name)
        except Exception as e:
            logger.warning(f"Could not delete physical file for upload {file_id}: {e}")
