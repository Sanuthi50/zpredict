...
def ask_question(self, student_id, question):
    ...
    try:
        ...
        try:
            ...
            # Enhanced prompting with comprehensive context handling
            if context:
                prompt = f"""
You are an expert educational assistant. Use the following context from the UGC Handbook to answer the student's question accurately.
If the context does not fully answer the question, answer based on general knowledge, but prioritize the context.

For casual greetings (like "hi", "hello", "how are you"), respond warmly and offer to help with educational questions.
    
Context (UGC Handbook):
{context}

Question:
{question}

Answer in a clear, student-friendly way. Include the source if needed.
"""
            else:
                prompt = f"""
You are an expert educational assistant. Answer the following student question using reliable online sources. 
If possible, provide references or indicate that the information is general knowledge.
