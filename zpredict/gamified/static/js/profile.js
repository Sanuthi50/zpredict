...
class StudentProfile {
    ...
    async checkAuth() {
        ...
        if (!token) {
            window.location.href = '/gamified/login/';
            return;
        }

        try {
            const response = await authenticatedFetch('http://127.0.0.1:8000/api/auth/me/');

            if (!response.ok) {
                console.error('Auth API response:', response.status, response.statusText);
    ...
    async loadFeedbacks() {
        try {
            const response = await authenticatedFetch('http://127.0.0.1:8000/api/feedback/');
    ...
    async deleteFeedback(feedbackId) {
        this.showConfirmationModal(
            ...
            async () => {
                try {
                    const response = await authenticatedFetch(`http://127.0.0.1:8000/api/feedback/${feedbackId}/`, {
                        method: 'DELETE'
                    });
    ...
    bindEvents() {
        ...
        document.getElementById('viewChatHistoryBtn').addEventListener('click', () => {
            window.location.href = '/gamified/history-chatbot/';
        });

        document.getElementById('viewPredictionsBtn').addEventListener('click', () => {
            window.location.href = '/gamified/predict/';
        });

        document.getElementById('viewCareerBtn').addEventListener('click', () => {
            window.location.href = '/gamified/recommendation/';
        });
    ...
    async updatePersonalInfo() {
        ...
        try {
            const response = await authenticatedFetch('http://127.0.0.1:8000/api/auth/update-profile/', {
                method: 'PUT',
    ...
    async changePassword() {
        ...
        try {
            const response = await authenticatedFetch('http://127.0.0.1:8000/api/auth/change-password/', {
                method: 'POST',
                body: JSON.stringify(data)
            });
    ...
    async submitFeedback() {
        ...
        try {
            const url = editId ? `http://127.0.0.1:8000/api/feedback/${editId}/` : 'http://127.0.0.1:8000/api/feedback/';
    ...
    deactivateAccount() {
        this.showConfirmationModal(
            ...
            async () => {
                try {
                    const response = await authenticatedFetch('http://127.0.0.1:8000/api/auth/deactivate-account/', {
                        method: 'POST'
                    });
    ...
    deleteAccount() {
        this.showConfirmationModal(
            ...
            async () => {
                try {
                    const response = await authenticatedFetch('http://127.0.0.1:8000/api/auth/delete-account/', {
                        method: 'DELETE'
                    });
