...
class AdminDashboard {
    ...
    renderModelHealth(data) {
        ...
        // Helper function to get status with better logic
        const getModelStatus = (loaded, instanceCreated) => {
            if (loaded) return 'healthy';
            if (instanceCreated) return 'warning'; // Instance exists but models not loaded (lazy loading)
            return 'error';
        };

        const getStatusText = (loaded, instanceCreated, type = 'model') => {
            if (loaded) return 'Loaded';
            if (instanceCreated) return 'Ready (Lazy Load)';
        ...
        const modelCards = [
            {
                ...
                status: getModelStatus(
                    data.models?.ml_prediction_models?.models_loaded,
                    data.models?.ml_prediction_models?.instance_created
                ),
                details: {
                    ...
                    'Regressor': data.models?.ml_prediction_models?.regressor_available ? 'Available' : 'Not Loaded',
                    'Classifier': data.models?.ml_prediction_models?.classifier_available ? 'Available' : 'Not Loaded'
                }
            },
            {
                ...
                icon: 'fas fa-rocket',
                status: getModelStatus(
                    data.models?.career_prediction_models?.models_loaded,
                    data.models?.career_prediction_models?.instance_created
                ),
                details: {
                    'Status': getStatusText(
                        data.models?.career_prediction_models?.models_loaded,
                    ...
                    'Data Available': data.models?.career_prediction_models?.hybrid_df_available ? 'Yes' : 'No'
                }
            },
            {
                title: 'Processing Status',
                icon: 'fas fa-cogs',
                status: data.uploads?.celery_worker_healthy ? 'healthy' : 'warning',
                details: {
                    'Celery Worker': data.uploads?.celery_worker_healthy ? 'Running' : 'Not Running',
                    'Pending': data.uploads?.pending_uploads || 0,
                    'Processing': data.uploads?.processing_uploads || 0,
                    'Completed': data.uploads?.completed_uploads || 0
                }
            },
            {
                title: 'Chat & Embeddings',
                icon: 'fas fa-comments',
                status: data.models?.embeddings_loaded || data.models?.vectorstore_count > 0 ? 'healthy' : 'warning',
                details: {
                    'Embeddings': data.models?.embeddings_loaded ? 'Loaded' : 'Ready (Lazy Load)',
                    'Vector Stores': data.models?.vectorstore_count || 0,
                    'LLM Model': data.models?.llm_loaded ? 'Loaded' : 'Ready (Lazy Load)'
                }
            }
        ];
